...
import "./RunsTestSuites.style.scss";
...
import { deleteRunsData, runsData } from "../../../Services/API/Run/Run";
...
const RunsTestSuites = ({
  ...
  const { runsListData, dataCount, currentOffset, isLoading, error } =
  ...
  // const runTestSuiteData = runData?.results;
  // const testSuitesCount = runData?.count;
  const [newrunTestSuiteData,setNewrunTestSuiteData] = useState()
  const [newtestSuitesCount,setNewtestSuitesCount]=useState()
  const [fetchsuiteRunlist,{isLoading: isRunsLoading,error: isRunsErrors}] = useLazyGetRunsTestCaseListQuery()
  const {socket} =  useContext(WebsocketContext)
  const currentStore = useStore()
  

  useEffect(() => {
    if (newtestSuitesCount) {
      setTotalPages(Math.ceil(newtestSuitesCount / rowsPerPage));
    }
  }, [newtestSuitesCount, rowsPerPage]);
  ...
  //       offset,
  //       selectedApplication,
  //       selectedProject,                                             // this is usefull for feature reference
  ...
  // fetch TestSuitesRuns

  const fetchTestSuitesRuns = async()=>{
    dispatch(setRunsLoading(true));
    try {
      const body =  {
        applicationId: selectedApplication?.id,
      ...
      const response = await fetchsuiteRunlist(body).unwrap()
      if(response){
        setNewrunTestSuiteData(response?.results)
        setNewtestSuitesCount(response?.count)
        dispatch(setRunTestCaseList(response?.results));
  ...
  useEffect(() => {
    ...
    console.log('type: ', type,selectedApplication);
    if (selectedApplication?.id && type === "TEST_SUITE") {
    ...
    startDate,
    endDate,
    isReClick,storedProjectId,storedApplicationId,storedUser,userData,userDetails
  ]);
  ...
  const handleDelete = async () => {
    ...
    //     setRowsPerPage(25);
    //     setPage(1);
    //     toast.success(message);              // this is usefull for feature reference
  ...
  const handleTestsuitesSocket = async(response)=>{
    ...
    if(currentProject?.id == response?.applicationId && currentProject?.projectId === response?.projectId ){
      if(response?.command == "createRun-suite"){
        const newTestSuiteRun = response?.data
        if(newTestSuiteRun){
          setNewrunTestSuiteData((preItem)=>{
            const newTestSuiteList = [...preItem, newTestSuiteRun];
    ...
    <>
      <div className="md:mt-4 run-Test-Cases run-Test-mobile-full bg-iwhite p-5 rounded-[8px] table-div">
        <div className="tableScroll tableScrollNew">
          <Table
            isLoading={isLoading}
            data={newrunTestSuiteData}
            onRowClick={(row) => {
              navigateTo(`/runs/test-suite/${row?._id}/summary`);
            }}
          ...
          <div className="flex items-center justify-between mt-8">
            {!isLoading && newtestSuitesCount > 0 && (
              <div className="text-sm text-ibl1">{`Showing ${newrunTestSuiteData?.length} out of ${newtestSuitesCount}`}</div>
            )}
            {!isLoading && newtestSuitesCount > 25 && (
