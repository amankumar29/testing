...
import TestPlanHealth from "../TestSuiteSummary/TestPlanHealth";
...
const TestPlanSummary = () => {
  ...
  console.log(runSummary, "runSummarytestplan");

  useEffect(() => {
    if (id) {
      dispatch(fetchRunSummary({ id, runType: "TEST_PLAN" }));
    }
  }, [id, dispatch]);
  ...
  return (
    <div data-testid="runs_Summary_Page" className="h-[calc(100vh-155px)] overflow-y-auto">
        ...
        <div className="items-center justify-between md:flex">
          <p className="flex items-center gap-3">
            <button
              ...
              onClick={() => navigateTo("/runs/test-scheduler")}
            >
              <ArrowBackIcon />
            </button>
      ...
      {step == 0 && <TestPlanHealth data={runSummary?.testSuites} />}
    </div>
  );
};
