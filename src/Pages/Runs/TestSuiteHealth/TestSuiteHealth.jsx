...
import styles from "./TestSuiteHealth.module.scss";
...
const TestSuiteHealth = ({ data = [] }) => {
  ...
  const handleTestSuiteSocket = async (response) => {
    try {
  ...
  useEffect(() => {
    if (socket) {
      socket.on("onTestcasesResponse", handleTestSuiteSocket);
      return () => {
        socket.off("onTestcasesResponse", handleTestSuiteSocket);
      };
    }
  }, []);
...
export default TestSuiteHealth;
